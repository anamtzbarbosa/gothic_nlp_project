...
def run_one_experiment(run, device, result_dir):
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"Grid Search V3 RNN Resume: {run_name}\n")
        f.write("=" * 80 + "\n")
...
def save_results(results, result_dir):
    ...
    with open(os.path.join(result_dir, "summary.txt"), "w") as f:
        f.write("Grid Search V3 — RNN (resumed)\n")
        f.write("=" * 80 + "\n")
        for r in ranked:
...
def parse_args():
    ...
    parser.add_argument("--seq-length", type=int, choices=[100, 200], default=None,
                        help="Fix seq_length. If not set, resumes both.")
...
def main():
    ...
    device = get_device()
    all_runs = create_grid(seq_lengths)
    results, done = load_existing_results(result_dir)
    remaining = [r for r in all_runs if r["name"] not in done]

    print(f"Using device: {device}")
    print(f"seq_length={seq_lengths} | Total: {len(all_runs)} | Done: {len(done)} | Remaining: {len(remaining)}\n")
