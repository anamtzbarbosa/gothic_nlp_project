...
# SELF-ATTENTION (our original working code)
class WindowedSelfAttention(nn.Module):
    ...
    def forward(self,lstm_output, window_size_k):
        ...
        if window_size_k > 0:
            mask = torch.triu(mask, diagonal=-window_size_k) 
...
# CROSS-ATTENTION (matches our proposal)
class CachedCrossAttention(nn.Module):
    ...
    def forward(self, lstm_output, cached_memory, window_size_k):
        ...
        if cached_memory is not None:
            full_context = torch.cat((cached_memory, lstm_output), dim=1)
        else:
            full_context = lstm_output
            
        memory_len = full_context.shape[1] - seq_len
        
        Q = self.W_q(lstm_output)
        K = self.W_k(full_context)
        V = self.W_v(full_context)
        
        K_transposed = K.transpose(1, 2) 
        attention_scores = torch.bmm(Q, K_transposed) / self.scale
        
        mask_seq = torch.tril(torch.ones((seq_len, seq_len), dtype=torch.bool, device=lstm_output.device), diagonal=-1)
        if memory_len > 0:
            mask_mem = torch.ones((seq_len, memory_len), dtype=torch.bool, device=lstm_output.device)
            mask = torch.cat((mask_mem, mask_seq), dim=1)
        else:
            mask = mask_seq
        ...
        context = torch.bmm(attention_weights, V)
        new_memory = full_context[:, -window_size_k:, :].detach()
        
        return context, attention_weights, new_memory
