...
RESULT_DIR = "results/grid_search_v3_rnn"
CHECKPOINT_DIR = "checkpoints/grid_search_v3_rnn"
...
def run_one_experiment(run, device):
    ...
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"Grid Search V3 RNN: {run_name}\n")
        f.write("=" * 80 + "\n")
...
def save_results(results):
    ...
    with open(os.path.join(RESULT_DIR, "summary.txt"), "w") as f:
        f.write("Grid Search V3 — RNN\n")
        f.write("=" * 80 + "\n")
        f.write("Searching: batch_size, seq_length, hidden_dim, learning_rate, dropout\n")
        f.write("Fixed: num_layers=1, embed_dim=128, vocab_size=5000\n")
        f.write("-" * 80 + "\n")
        for r in ranked:
...
def main():
    ...
    print(f"Searching: batch_size=[32,64], seq_length=[100,200], hidden_dim=[128,256]")
    print(f"           learning_rate=[5e-4,1e-3], dropout=[0.0,0.2,0.3]")
    print(f"Fixed: num_layers=1, embed_dim=128, vocab_size=5000\n")
